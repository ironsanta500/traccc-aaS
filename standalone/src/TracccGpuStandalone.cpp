...
int main(int argc, char *argv[])
{
    ...
    if (argc >=4){
        std::cout << "Pipeline mode: " << argv[3] << std::endl;
    } else {
        std::cout << "Pipeline mode: TRACK_FINDING (default)" << std::endl;
    }
    ...
    std::string pipeline_mode_str = "TRACK_FINDING"; // default
    if (argc >= 4) {
        pipeline_mode_str = std::string(argv[3]);
    }

    PipelineMode pipeline_mode;
    if (pipeline_mode_str == "CLUSTERIZATION") {
        pipeline_mode = PipelineMode::CLUSTERIZATION;
    } else if (pipeline_mode_str == "TRACK_FINDING") {
        pipeline_mode = PipelineMode::TRACK_FINDING;
    } else {
        std::cerr << "Invalid pipeline mode: " << pipeline_mode_str << std::endl;
        return -1;
    }

    // auto traccc_result = traccc_gpu.run(cells, true);
    auto traccc_result = traccc_gpu.run(cells, pipeline_mode, false);

    if (pipeline_mode == PipelineMode::CLUSTERIZATION) {
        std::cout << "Number of clusterization measurements: " << traccc_result.measurements.size() << std::endl;
    } else if (pipeline_mode == PipelineMode::SPACEPOINT_FORMATION) {
        std::cout << "Number of spacepoints: " << traccc_result.spacepoints.size() << std::endl;
    } else if (pipeline_mode == PipelineMode::SEEDING) {
        std::cout << "Number of seeds: " << traccc_result.seeds.size() << std::endl;
    }

    if (pipeline_mode == PipelineMode::TRACK_FINDING) {
