...
def main():
    ...
    # Specify outputs
    output_map = {
        "traccc-cuda-clusterization": ["MEASUREMENTS", "GEOMETRY_IDS"],
        "traccc-cuda-spacepoints":    ["SPACEPOINTS"],
        "traccc-cuda-seeding":        ["SPACEPOINTS", "SEEDS"],
        "traccc-cuda-tracking":       ["TRK_PARAMS", "MEASUREMENTS", "COVARIANCES", "GEOMETRY_IDS"],
    }
    output_names = output_map[FLAGS.model]
    ...
    if FLAGS.model == "traccc-cuda-clusterization":
            measurements = result.as_numpy("MEASUREMENTS")
            geometry_ids = result.as_numpy("GEOMETRY_IDS")
            print(f"Number of measurements: {len(measurements)}")
            print(f"Number of geometry IDs: {len(geometry_ids)}")

    elif FLAGS.model == "traccc-cuda-spacepoints":
        spacepoints = result.as_numpy("SPACEPOINTS")
        print(f"Number of spacepoints: {len(spacepoints)}")

    elif FLAGS.model == "traccc-cuda-seeding":
        spacepoints = result.as_numpy("SPACEPOINTS")
        seeds = result.as_numpy("SEEDS")
        print(f"Number of spacepoints: {len(spacepoints)}")
        print(f"Number of seeds: {len(seeds)}")

    elif FLAGS.model == "traccc-cuda-tracking":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="traccc-cuda-tracking",
        help="Model name. Options: traccc-cuda-clusterization, traccc-cuda-spacepoints, traccc-cuda-seeding, traccc-cuda-tracking",
    )
